...
location to the nearest monster. Essientally, you BFS from each monster
simultaneously to find how far each open spot is from it's closest monster.
Knowing this, we may BFS to find all locations that are visitable by A. This 
will run in $N^2$ time because each location will be visited a constant amount of 
times.
